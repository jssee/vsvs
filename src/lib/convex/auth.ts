...
const siteUrl = (() => {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    console.warn(
      `SITE_URL is not set; falling back to ${fallback}. Set SITE_URL to your app URL for consistent Better Auth behaviour.`,
    );
    return fallback;
  }

  throw new Error(
    "SITE_URL environment variable is not set. Provide the application's base URL to configure Better Auth.",
  );
})();
