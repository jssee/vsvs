...
<div class="mx-auto max-w-3xl space-y-6 p-4">
  ...
  <header class="rounded border p-4">
    <div class="flex items-baseline justify-between">
      <h1 class="text-xl font-semibold">
        Session {data.session.sessionNumber}: {data.session.vibe}
      </h1>
      <span class="text-xs text-gray-600">{data.session.phase}</span>
    </div>
    ...
    {#if data.session.playlistUrl}
      <div class="mt-2 text-sm">
        Playlist:
        <a class="underline" href={data.session.playlistUrl} target="_blank"
          >{data.session.playlistUrl}</a
        >
      </div>
    {/if}
  ...
  {#if data.session.phase === "submission"}
